...
export default function CourseContent({
  ...
  const handleVideoCompleted = async () => {
    ...
    try {
      // 1. Mark video as completed via API
      ...
      if (moduleIndex !== -1) {
        ...
        if (videoIndex !== -1) {
          ...
          // 3. Update parent state immediately for instant UI feedback
          if (onCourseUpdate) {
      ...
      // 4. Check if all videos in module are completed
      const currentModule = updatedCourse.modules?.find(m => m.id === currentModuleId);
      const allVideosCompleted = currentModule?.videos?.every(v => !v.isLocked && v.isCompleted);

      if (allVideosCompleted) {
        console.log("🎉 All videos completed, unlocking assessment");
        try {
          await unlockAssessment(currentModuleId);
        ...
        const currentModuleIndex = updatedCourse?.modules?.findIndex(m => m.id === currentModuleId);
        if (currentModuleIndex >= 0 && currentModuleIndex < updatedCourse.modules.length - 1) {
          const nextModule = updatedCourse.modules[currentModuleIndex + 1];
          if (nextModule?.isLocked) {
            console.log("🔓 Unlocking next module...");
            try {
      ...
      // 5. Do a full refresh to sync with backend
      setTimeout(async () => {
        console.log("🔄 Doing final refresh to sync with backend");
        await refreshCourse();
        setShowNextOverlay(false);

        // 6. Auto-play next video if available
        const freshModule = course.modules?.find(m => m.id === currentModuleId);
        if (freshModule?.videos) {
          for (let vi = currentVideoIndex + 1; vi < freshModule.videos.length; vi++) {
            const nextVideo = freshModule.videos[vi];
